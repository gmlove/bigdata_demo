...
public class MainTest {
    private static final MessageType SCHEMA = Types.buildMessage()
            ...
            .requiredList().requiredElement(INT64).as(INT_64).named("phone_numbers")
            .optional(BINARY).as(UTF8).named("comment")
            .named("msg");
    private static final Comparator<Binary> BINARY_COMPARATOR = Types.required(BINARY).as(UTF8).named("dummy")
            .comparator();
...
//    @Test
    public void testSimpleWriteRead() throws Throwable {
        List<Group> data = Stream.generate(new DataGenerator(0)).limit(100).collect(Collectors.toList());
        Path file = writeFile(data);
        validateFile(file, data);
        validateFileWithIdFilter(file, data);
    }
//    @Test
